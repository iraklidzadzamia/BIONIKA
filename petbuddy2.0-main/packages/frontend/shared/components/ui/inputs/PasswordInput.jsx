"use client";
import React, { useState, useMemo } from "react";
import {
  EyeIcon,
  EyeSlashIcon,
  LockClosedIcon,
} from "@heroicons/react/24/outline";

const PasswordInput = React.forwardRef(
  (
    {
      className = "",
      error,
      label,
      required = false,
      placeholder = "Enter password",
      showStrength = false,
      ...props
    },
    ref
  ) => {
    const [showPassword, setShowPassword] = useState(false);

    // Calculate password strength and requirements
    const validation = useMemo(() => {
      const password = props.value || "";
      if (!showStrength) return null;

      const requirements = {
        minLength: password.length >= 8,
        hasLowercase: /[a-z]/.test(password),
        hasUppercase: /[A-Z]/.test(password),
        hasNumber: /\d/.test(password),
      };

      const metCount = Object.values(requirements).filter(Boolean).length;
      let strength;

      if (metCount <= 1) {
        strength = { level: "weak", label: "Weak", color: "red" };
      } else if (metCount <= 2) {
        strength = { level: "medium", label: "Medium", color: "yellow" };
      } else {
        strength = { level: "strong", label: "Strong", color: "green" };
      }

      return { requirements, strength };
    }, [props.value, showStrength]);

    const baseClasses =
      "w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-primary-100 focus:border-primary-500 transition-all duration-200 text-base placeholder-neutral-400 bg-white";

    const errorClasses = error
      ? "border-red-300 focus:ring-red-100 focus:border-red-500"
      : "border-gray-200 hover:border-gray-300";

    const finalClasses = `${baseClasses} ${errorClasses} ${className}`;

    return (
      <div className="relative">
        {label && (
          <label className="block text-sm font-medium text-gray-700 mb-2 ml-1 leading-5">
            {label}
            {required && <span className="text-red-500 ml-1">*</span>}
          </label>
        )}
        <div className="relative">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
            <LockClosedIcon className="w-5 h-5" />
          </div>
          <input
            ref={ref}
            type={showPassword ? "text" : "password"}
            className={`${finalClasses} pl-12 pr-12 h-12`}
            placeholder={placeholder}
            {...props}
          />
          <button
            type="button"
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors"
            onClick={() => setShowPassword(!showPassword)}
            tabIndex={-1}
          >
            {showPassword ? (
              <EyeSlashIcon className="w-5 h-5" />
            ) : (
              <EyeIcon className="w-5 h-5" />
            )}
          </button>
        </div>
        {validation && (
          <div className="mt-2 space-y-2">
            {/* Strength Bar */}
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-300 ${
                    validation.strength.color === "red"
                      ? "bg-red-500 w-1/3"
                      : validation.strength.color === "yellow"
                      ? "bg-yellow-500 w-2/3"
                      : "bg-green-500 w-full"
                  }`}
                />
              </div>
              <span
                className={`text-xs font-medium ${
                  validation.strength.color === "red"
                    ? "text-red-600"
                    : validation.strength.color === "yellow"
                    ? "text-yellow-600"
                    : "text-green-600"
                }`}
              >
                {validation.strength.label}
              </span>
            </div>

            {/* Requirements Checklist */}
            {props.value && (
              <ul className="text-xs space-y-1">
                <li
                  className={
                    validation.requirements.minLength
                      ? "text-green-600"
                      : "text-gray-500"
                  }
                >
                  {validation.requirements.minLength ? "✓" : "○"} At least 8
                  characters
                </li>
                <li
                  className={
                    validation.requirements.hasLowercase
                      ? "text-green-600"
                      : "text-gray-500"
                  }
                >
                  {validation.requirements.hasLowercase ? "✓" : "○"} One
                  lowercase letter
                </li>
                <li
                  className={
                    validation.requirements.hasUppercase
                      ? "text-green-600"
                      : "text-gray-500"
                  }
                >
                  {validation.requirements.hasUppercase ? "✓" : "○"} One
                  uppercase letter
                </li>
                <li
                  className={
                    validation.requirements.hasNumber
                      ? "text-green-600"
                      : "text-gray-500"
                  }
                >
                  {validation.requirements.hasNumber ? "✓" : "○"} One number
                </li>
              </ul>
            )}
          </div>
        )}

        {error && (
          <p className="mt-2 text-sm text-red-600 flex items-center gap-1">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                clipRule="evenodd"
              />
            </svg>
            {error}
          </p>
        )}
      </div>
    );
  }
);

PasswordInput.displayName = "PasswordInput";

export default PasswordInput;
