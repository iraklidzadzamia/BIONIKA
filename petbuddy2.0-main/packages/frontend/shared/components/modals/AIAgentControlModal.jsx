...
import { Bot, BotOff, Clock, Calendar, AlertCircle } from "lucide-react";
import { Button } from "@/shared/components/ui";

// Content-only modal for use with ModalRoot
// Expects ModalRoot to provide title/close via ui config and onClose prop
export default function AIAgentControlModal({
  ...
  const handleSave = async () => {
    ...
    // If onSave returns a promise, wait for it and close on success
    if (result && typeof result.then === "function") {
      try {
        await result;
        onClose?.();
      } catch (_) {
        return; // Parent handles error feedback; keep modal open
      }
    } else {
      // Non-promise handler treated as success
      onClose?.();
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header context (ModalRoot provides title/close) */}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-900">
            ...
            <ul className="space-y-1 text-xs">
              <li>• When active, the AI responds automatically to messages</li>
              <li>• When suspended, you handle all messages manually</li>
              <li>• Set a resume schedule to temporarily suspend the agent</li>
